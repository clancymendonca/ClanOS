...
**Last verified locally (2026-06-21):** scope-464 seed migration — `sysinfo` → `trust=system-signed`; gate `2.4.0` (**4 signed**, **12 digest-only remaining** of 16 seeds).
...
- **ADR-0003 loader signed exec (epoch 460):** kernel `loader_signed_exec.rs` + `loader_signed_exec_integration` (12 cases); seed migration **4 signed / 12 digest-only remaining** of 16 (`demo-hello`, `echo`, `time`, `sysinfo`; sunset scope 465)
