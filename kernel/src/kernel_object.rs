...
pub fn cap_transfer_move(
    ...
    // Threat class: resource leak only. Orphaned cap is unreachable by any process.
    // Not a privilege escalation or cap duplication. Object generation unaffected.
    // Fix requires: reserved-slot protocol (source slot marked Reserved before close,
    // restored to Active on alloc failure). See docs/specs/CAP_TRANSFER_PROTOCOL.md
    // when that doc reaches canonical status in Batch 2.
