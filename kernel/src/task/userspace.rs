...
pub fn run_program(name: &str, args: &[&str]) -> Result<String, &'static str> {
    ...
    let program = match crate::task::program_loader::resolve_program_for(credentials, name) {
        ...
        Err(crate::task::program_loader::ProgramLoadError::UnsupportedExecution) => {
            if crate::task::program_loader::hw_user_elf_ready()
                && crate::task::program_loader::ALLOWED_USER_ELFS.contains(&name)
            {
                match crate::task::program_loader::execute_allowlisted_user_elf(credentials, name) {
                    Ok(execution) => {
...
fn dispatch_builtin(entry: &str, args: &[&str]) -> Result<String, &'static str> {
    match entry {
        ...
        "demo-hello" => Ok(String::from("clan-rt demo: hello from userland\n")),
        "clan-info" => dispatch_builtin("sysinfo", args),
...
fn is_builtin_entry(name: &str) -> bool {
    matches!(
        name,
        "echo" | "time" | "sysinfo" | "fsinfo" | "demo-hello" | "clan-info"
    )
}
