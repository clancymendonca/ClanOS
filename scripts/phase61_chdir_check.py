...
PHASE_RE = re.compile(r"Phase61-Chdir:\s+normalized=(\d+),\s+chdirs=(\d+),\s+ok=(true|false)")
...
def ok(output):
    for line in output.splitlines():
        ...
        if m:
            normalized, chdirs, flag = m.groups()
            return int(normalized) >= 1 and int(chdirs) >= 1 and flag == "true"
    return False
