...
PHASE_RE = re.compile(r"Phase56-MultiShlib:\s+loaded=(\d+),\s+pages=(\d+),\s+ok=(true|false)")
...
def ok(output):
    for line in output.splitlines():
        ...
        if m:
            loaded, pages, flag = m.groups()
            return int(loaded) >= 2 and int(pages) >= 2 and flag == "true"
    return False
