...
PHASE_RE = re.compile(r"Phase79-ApTrampoline:\s+aps=(\d+),\s+idle_ticks=(\d+),\s+ok=(true|false)")
...
def ok(output):
    for line in output.splitlines():
        ...
        if m:
            aps, idle_ticks, flag = m.groups()
            return int(aps) >= 1 and int(idle_ticks) >= 1 and flag == "true"
    return False
