...
PHASE_RE = re.compile(r"Phase52-FdDup:\s+dups=(\d+),\s+relative=(\d+),\s+ok=(true|false)")
...
def ok(output):
    for line in output.splitlines():
        ...
        if m:
            dups, relative, flag = m.groups()
            return int(dups) >= 1 and int(relative) >= 1 and flag == "true"
    return False
