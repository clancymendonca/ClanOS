...
PHASE_RE = re.compile(r"Phase64-ForkLite:\s+inherited=(\d+),\s+isolated=(\d+),\s+ok=(true|false)")
...
def ok(output):
    for line in output.splitlines():
        ...
        if m:
            inherited, isolated, flag = m.groups()
            return int(inherited) >= 1 and int(isolated) >= 1 and flag == "true"
    return False
