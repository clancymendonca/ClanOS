...
PHASE_RE = re.compile(r"Phase66-Fcntl:\s+getfd=(\d+),\s+dupfd=(\d+),\s+rejected=(\d+),\s+ok=(true|false)")
...
def ok(output):
    for line in output.splitlines():
        ...
        if m:
            getfd, dupfd, rejected, flag = m.groups()
            return int(getfd) >= 1 and int(dupfd) >= 1 and int(rejected) >= 1 and flag == "true"
    return False
