...
PHASE_RE = re.compile(r"Phase43-TrustExec:\s+executed=(\d+),\s+rejected=(\d+),\s+ok=(true|false)")
def cleanup():
    if os.name == "nt":
...
def ok(output):
    for line in output.splitlines():
        ...
        if m:
            executed, rejected, flag = m.groups()
            return int(executed) >= 1 and int(rejected) >= 1 and flag == "true"
    return False
def main():
