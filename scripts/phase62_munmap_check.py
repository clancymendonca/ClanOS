...
PHASE_RE = re.compile(r"Phase62-Munmap:\s+unmapped=(\d+),\s+rejected=(\d+),\s+ok=(true|false)")
...
def ok(output):
    for line in output.splitlines():
        ...
        if m:
            unmapped, rejected, flag = m.groups()
            return int(unmapped) >= 2 and int(rejected) >= 1 and flag == "true"
    return False
