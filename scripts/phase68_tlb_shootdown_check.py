...
PHASE_RE = re.compile(r"Phase68-TlbShootdown:\s+cpus=(\d+),\s+shootdowns=(\d+),\s+ok=(true|false)")
...
def ok(output):
    for line in output.splitlines():
        ...
        if m:
            cpus, shootdowns, flag = m.groups()
            return int(cpus) >= 2 and int(shootdowns) >= 2 and flag == "true"
    return False
