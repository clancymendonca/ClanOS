...
PHASE_RE = re.compile(r"Phase74-WaitLite:\s+waited=(\d+),\s+rejected=(\d+),\s+ok=(true|false)")
...
def ok(output):
    for line in output.splitlines():
        ...
        if m:
            waited, rejected, flag = m.groups()
            return int(waited) >= 1 and int(rejected) >= 1 and flag == "true"
    return False
