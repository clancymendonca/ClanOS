...
PHASE_RE = re.compile(r"Phase42-DynReloc:\s+imports=(\d+),\s+applied=(\d+),\s+ok=(true|false)")
def cleanup():
    if os.name == "nt":
...
def ok(output):
    for line in output.splitlines():
        ...
        if m:
            imports, applied, flag = m.groups()
            return int(imports) >= 1 and int(applied) >= 1 and flag == "true"
    return False
def main():
