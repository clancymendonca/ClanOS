...
PHASE_RE = re.compile(
    r"Phase53-Mprotect:\s+applied=(\d+),\s+rejected=(\d+),\s+guard_faults=(\d+),\s+ok=(true|false)"
)
...
def ok(output):
    for line in output.splitlines():
        ...
        if m:
            applied, rejected, guard, flag = m.groups()
            return int(applied) >= 1 and int(guard) >= 1 and flag == "true"
    return False
