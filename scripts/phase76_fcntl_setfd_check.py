...
PHASE_RE = re.compile(
    r"Phase76-Fcntl:\s+setfd=(\d+),\s+getfd=(\d+),\s+rejected=(\d+),\s+ok=(true|false)"
)
...
def ok(output):
    for line in output.splitlines():
        ...
        if m:
            setfd, getfd, rejected, flag = m.groups()
            return int(setfd) >= 1 and int(getfd) >= 1 and int(rejected) >= 1 and flag == "true"
    return False
