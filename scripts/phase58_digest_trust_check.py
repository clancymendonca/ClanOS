...
PHASE_RE = re.compile(r"Phase58-DigestTrust:\s+verified=(\d+),\s+rejected=(\d+),\s+ok=(true|false)")
...
def ok(output):
    for line in output.splitlines():
        ...
        if m:
            verified, rejected, flag = m.groups()
            return int(verified) >= 1 and int(rejected) >= 1 and flag == "true"
    return False
