...
PHASE_RE = re.compile(r"Phase57-PltReloc:\s+slots=(\d+),\s+applied=(\d+),\s+ok=(true|false)")
...
def ok(output):
    for line in output.splitlines():
        ...
        if m:
            slots, applied, flag = m.groups()
            return int(slots) >= 1 and int(applied) >= 1 and flag == "true"
    return False
