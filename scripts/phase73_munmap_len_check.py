...
PHASE_RE = re.compile(
    r"Phase73-MunmapLen:\s+unmapped_pages=(\d+),\s+partial_regions=(\d+),\s+ok=(true|false)"
)
...
def ok(output):
    for line in output.splitlines():
        ...
        if m:
            pages, partial, flag = m.groups()
            return int(pages) >= 3 and int(partial) >= 1 and flag == "true"
    return False
