...
PHASE_RE = re.compile(r"Phase71-Sysret:\s+probes=(\d+),\s+sysret_ok=(true|false),\s+ok=(true|false)")
...
def ok(output):
    for line in output.splitlines():
        ...
        if m:
            probes, sysret_ok, flag = m.groups()
            return int(probes) >= 1 and sysret_ok == "true" and flag == "true"
    return False
