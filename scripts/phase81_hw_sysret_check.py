...
PHASE_RE = re.compile(
    r"Phase81-HwSysret:\s+probes=(\d+),\s+sysret_real=(true|false),\s+ok=(true|false)"
)
...
def ok(output):
    for line in output.splitlines():
        ...
        if m:
            probes, sysret_real, flag = m.groups()
            return int(probes) >= 1 and sysret_real == "true" and flag == "true"
    return False
