...
PHASE_RE = re.compile(r"Phase59-Runqueues:\s+cpus=(\d+),\s+enqueued=(\d+),\s+ok=(true|false)")
...
def ok(output):
    for line in output.splitlines():
        ...
        if m:
            cpus, enqueued, flag = m.groups()
            return int(cpus) >= 2 and int(enqueued) >= 1 and flag == "true"
    return False
