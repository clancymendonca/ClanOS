...
PHASE_RE = re.compile(r"Phase51-ProcFd:\s+isolated=(true|false),\s+opens=(\d+),\s+ok=(true|false)")
...
def ok(output):
    for line in output.splitlines():
        ...
        if m:
            isolated, opens, flag = m.groups()
            return isolated == "true" and int(opens) >= 1 and flag == "true"
    return False
