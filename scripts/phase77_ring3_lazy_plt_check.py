...
PHASE_RE = re.compile(
    r"Phase77-Ring3LazyPlt:\s+lazy=(\d+),\s+bound=(\d+),\s+ring3=(\d+),\s+ok=(true|false)"
)
...
def ok(output):
    for line in output.splitlines():
        ...
        if m:
            lazy, bound, ring3, flag = m.groups()
            return int(lazy) >= 1 and int(bound) >= 1 and int(ring3) >= 1 and flag == "true"
    return False
