...
See: [FAULT_ESCALATION.md](FAULT_ESCALATION.md), [KERNEL_OBJECT_MODEL.md](KERNEL_OBJECT_MODEL.md), [DECISION_LOG.md](../../DECISION_LOG.md).

---

## Cap handles

Scheduler operates on **cap handles**, not raw object mutation. See [KERNEL_OBJECT_MODEL.md](KERNEL_OBJECT_MODEL.md) § Overview.
...
Tier-3 fault escalation when revoke occurs during blocked wait: [FAULT_ESCALATION.md](FAULT_ESCALATION.md) §Tier 3.

```mermaid
stateDiagram-v2
    [*] --> Runnable
    Runnable --> BlockedOnCap : wait_on_C
    BlockedOnCap --> Runnable : C_valid_at_checkpoint
    BlockedOnCap --> Terminal : C_revoked_at_checkpoint
    Terminal --> [*]
```
...
Delegation-chain revocation only. Parent revoke invalidates subtree at checkpoint (depth-first). Not object destruction (`R-destroy-notify`). See [KERNEL_OBJECT_MODEL.md](KERNEL_OBJECT_MODEL.md) § Revocation models.
...
No priority inheritance chains beyond this ceiling rule. Tier-3 escalation on deadlock: [FAULT_ESCALATION.md](FAULT_ESCALATION.md) §Tier 3.
