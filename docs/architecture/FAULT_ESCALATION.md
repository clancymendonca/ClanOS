...
```mermaid
stateDiagram-v2
    [*] --> Tier1 : transient_fault
    Tier1 --> Tier1 : retry_ok
    Tier1 --> Tier2 : service_unrecoverable
    Tier2 --> Tier1 : restart_ok
    Tier2 --> Tier3 : handler_exhausted
    Tier1 --> Tier3 : security_violation
    Tier3 --> [*] : halt_or_reboot
```

Tier-3 escalation path coordinates with [SCHEDULER_MODEL.md](SCHEDULER_MODEL.md) §R-revoke-blocked at authority checkpoints.
...
**R-revoke-blocked:** thread blocked in syscall on cap C — when C revoked, wait ends with **terminal error** at checkpoint. Scheduler behavior: [SCHEDULER_MODEL.md](SCHEDULER_MODEL.md) §R-revoke-blocked.
...
| State | Milestone 150 |
|-------|---------------|
| **Suspend** (phase 147) | Frozen-in-memory; no persistent checkpoint |
