...
# Scope 5 Checklist (Preemptive Scheduling & Process Foundation)
...
Scope 5 builds on Scope 4's context switching infrastructure to establish:
...
5. **System Calls**: Foundation for user-mode interaction (prep for Scope 6)
...
- [x] Runtime configuration support for scheduler parameters (API-based)
  ...
  - `FAIRNESS_CHECK_INTERVAL_TICKS` (from Scope 4, verify still working)
...
## Deferred Hardening (Scope 6 Candidate)
...
- Single-core focus; multi-core support deferred to Scope 6
- User-mode processes & address spaces deferred to Scope 6 (only kernel tasks in Scope 5)
- System calls API design begins in Scope 5; implementation in Scope 6
- Scope 4's `context-lab` feature is the foundation; replaces it with stable `preemption` flag
- **Day 1 Completed**: 
  ...
  - ✅ All Scope 4 tests passing (27/27)
...
- `python scripts/preemption/soak.py --duration 600 --min-samples 10` ✅ (passed 2026-03-23)
- `python scripts/preemption/latency.py --duration 120 --min-samples 5 --max-latency-ms 100` ✅ (passed 2026-03-23)
...
**Next Step**: Scope 6 user-space foundation and shell bring-up.
