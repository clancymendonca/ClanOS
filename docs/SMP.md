...
Scope 49 adds multiprocessor detection and hooks for future TLB management without scheduling work on application processors yet.
...
Scope 31 binds CR3 on preemptive context switch on the bootstrap processor only. Scope 49 does not migrate runnable tasks to APs.
...
## TLB Shootdown Accounting (Scope 68)
...
## IPI TLB Shootdown Stub (Scope 78)

`request_tlb_shootdown()` records logical IPI send/ack counters (`ipi_shootdown_sent`, `ipi_shootdown_acked`) for all online CPUs, then flushes the BSP TLB. Munmap (scope 73) calls this helper after PTE changes.
...
## AP Runnable Enqueue Stub (Scope 98)
