...
## Syscall / register ABI — `ares-abi-v1`

| Property | Value |
|----------|--------|
| Status | Frozen at phase 101 for compat IDs 1–82 |
...
Bump to `ares-abi-v2` requires: migration notes, compat shim period, matrix smoke updates.

---

## Semantic laws — `ares-semantics-v1` (draft at phase 109)

| Property | Value |
|----------|--------|
| Status | Draft ratified phase 110 |
...
Bump to `ares-semantics-v2` requires:

1. [SEMANTIC_SPECS.md](SEMANTIC_SPECS.md) case updates
2. [SEMANTIC_LINT.md](SEMANTIC_LINT.md) CI pass (phases 159–160+)
3. Minimization audit (A10)
4. `ares-semantics-v*` noted in release notes
...
- [ ] `ares-abi-v1` compat table matches `ALLOWED_HW_SYSCALLS`
- [ ] `ares-semantics-v1` draft covers IPC + rights + temporal outlines
